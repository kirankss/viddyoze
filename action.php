...
//check Offers on cart item
if (isset($_POST["check_offers"])) {
    //When user is logged in then we will check offers by using user session id
    if (isset($_SESSION["uid"])) {
        //When user is logged in this query will execute
        $sql = "SELECT a.product_id,a.offer_id,a.offer_name,a.offer_minqty,a.offer_value,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.user_id='$_SESSION[uid]'";
    } else {
        //When user is not logged in this query will execute
        $sql = "SELECT a.product_id,a.offer_id,a.offer_name,a.offer_minqty,a.offer_value,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.ip_add='$ip_add' AND b.user_id < 0";
    }
    
    $query = mysqli_query($con, $sql);
    $row = mysqli_fetch_array($query);
    $offer_minqty = $row["offer_minqty"];
    $offer_value = $row["offer_value"];
    $qty = $row["qty"];

    $finalqty = $qty;

    if (($qty - $offer_minqty) > 0) {
        $finalqty = $qty - $offer_minqty + $offer_value;
    }

    echo $finalqty;
    //echo $row["check_offers"];
    exit();
}
//check Offers on cart item
...
if (isset($_POST["Common"])) {
    if (isset($_SESSION["uid"])) {
        ...
        //When user is not logged in this query will execute
        $sql = "SELECT a.*,b.id,b.qty FROM products a,cart b WHERE a.product_id=b.p_id AND b.ip_add='$ip_add' AND b.user_id < 0";
    }
    $query = mysqli_query($con, $sql);
    if (isset($_POST["getCartItem"])) {
        ...
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($row=mysqli_fetch_array($query)) {
                ...
                if ($qty >= $offer_minQty) {
                    $product_total_price = ($qty - $offer_Value) * $product_price;
                } else {
                ...
                echo '
                    <div class="product-widget">
												...
												<div class="product-body">
													<h3 class="product-name"><a href="#">'.$n.'. '.$product_title.'</a></h3>
													<input type="hidden" class="price" value="'.$product_price.'">
                                                    <input type="hidden" class="qty" value="'.$qty.'">
                                                    <input type="hidden" class="offer_Value" value="'.$offer_Value.'">
													<input type="hidden" class="offer_minQty" value="'.$offer_minQty.'">
                                                    <input type="hidden" class="cart_total" value="'.$product_total_price.'">
													<h4 class="product-price">$'.$product_price.'</h4>
												</div>												
											</div>'
                    ;
            }
            $shipping_charge_sql = "SELECT * FROM shipping_charges order by min_total_cost";
            $shipping =  mysqli_query($con, $shipping_charge_sql);
            $shipping_charges = 0;
            if ($shipping) {
                if (mysqli_num_rows($shipping) > 0) {
                    while ($row=mysqli_fetch_array($shipping)) {
                        if ($total_price > $row["min_total_cost"] && $total_price < $row["max_total_cost"]) {
                            $shipping_charges = $row["shipping_charge"];
                        }
                    }
                }
            }
            $total_price += $shipping_charges;
            
            echo '<div class="cart-summary">
				    <small class="qty1">'.$n.' Item(s) selected</small>
				    <h5><b class="net_total" >Total : $ '.$total_price.'</b></h5>
				</div>'
            ?>
    ...
    if (isset($_POST["checkOutDetails"])) {
        if (mysqli_num_rows($query) > 0) {
            ...
            while ($row=mysqli_fetch_array($query)) {
                ...
                $product_total_price = round($product_total_price, 3);
                ...
                echo
						...
						<tr>
							...
							<td data-th="Subtotal" class="text-center"><input type="text" class="form-control total" value="'.$product_total_price.'" readonly="readonly"></td>
            ...
            echo '</tbody>
				<tfoot>
					...
					<tr>
						...
						<td colspan="3" class="hidden-xs text-right"><b class="net_total" ></b></td>
...
if (isset($_POST["shipping_charges"])) {
    $sql = "SELECT * FROM shipping_charges order by min_total_cost";
    $shipping =  mysqli_query($con, $sql);
