$(document).ready(function() {
  ...
  function updateAmount(obj) {
    ...
    var offer_Value = row.find('.offer_Value').val();
    var offer_minQty = row.find('.offer_minQty').val();
    if (isNaN(qty)) {
      qty = 1;
    }
    if (qty < 1) {
    ...
    $.ajax({
      url     : 'action.php',
      method  : 'POST',
      data    : { shipping_charges: 1 },
      success : function(data) {
        let list_shipping_charges = JSON.parse(data);
        let shipping_charges = 0;
        for (let index = 0; index < list_shipping_charges.length; index++) {
          const element = list_shipping_charges[index];
          if (
            parseFloat(net_total) > parseFloat(element.min_total_cost) &&
            parseFloat(net_total) < parseFloat(element.max_total_cost)
          ) {
            shipping_charges = element.shipping_charge;
            break;
          }
        }
        net_total = parseFloat(net_total) + parseFloat(shipping_charges);
  ...
  /*
	...
	*/
  function net_total(status = false) {
    $('.qty').each(function() {
      updateAmount(this);
    });
  }
